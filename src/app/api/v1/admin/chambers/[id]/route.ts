...
/**
 * PATCH /api/v1/admin/chambers/[id] — admin updates any chamber
 * (including activation/deactivation). No ownership restriction.
 */
export const PATCH = withErrorHandler(async (req: NextRequest, { params }: { params: { id: string } }) => {
  const start = nowMs();
  const path = "/api/v1/admin/chambers/[id]";
  try {
    ...
    const chamber = await prisma.chamber.findFirst({ where: { id: chamberId, deletedAt: null } });
    ...
    if (d.hospitalId !== undefined && d.hospitalId !== null) {
      const hospital = await prisma.hospital.findFirst({
        where: { id: d.hospitalId, deletedAt: null },
        select: { id: true },
      });
    ...
    await prisma.auditLog.create({
      data: { userId: ctx.userId, action: "CHAMBER_UPDATED", entity: "Chamber", entityId: chamberId, meta: { changed: Object.keys(data) } },
    });
...
/** DELETE /api/v1/admin/chambers/[id] — soft-delete a chamber (admin only). */
export const DELETE = withErrorHandler(async (req: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    await prisma.auditLog.create({
      data: { userId: ctx.userId, action: "CHAMBER_DELETED", entity: "Chamber", entityId: chamberId },
    });
