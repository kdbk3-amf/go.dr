...
export const GET = withErrorHandler(async (req: NextRequest) => {
  ...
  try {
    rateLimit(`${getClientIp(req)}:admin-users`, DEFAULT_RATE_LIMIT);
    ...
    const [users, total] = await Promise.all([
      ...
      prisma.user.count({ where: { deletedAt: null, ...(roleParam ? { role: roleParam } : {}) } }),
    ]);

    logRequest("GET", path, 200, nowMs() - start);
    void ctx;
    return ok({ users: toJsonSafe(users) }, 200, { page, limit, total, totalPages: Math.ceil(total / limit) });
  } catch (err) {
