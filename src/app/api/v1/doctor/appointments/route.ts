 ...
 * Authenticated DOCTOR only. Lists appointments belonging to the
 * calling doctor, with optional status + upcoming filters, pagination
 * and sorting. Never exposes another doctor's appointments.
 */
export const GET = withErrorHandler(async (req: NextRequest) => {
  const start = nowMs();
  const path = "/api/v1/doctor/appointments";
  try {
